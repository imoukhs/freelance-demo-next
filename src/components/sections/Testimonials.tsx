import { NicheConfig } from "@/lib/types/niche";

interface Props {
  config: NicheConfig;
}

export default function Testimonials({ config }: Props) {
  return (
    <section className="py-24 px-5" style={{ backgroundColor: "var(--color-bg)" }}>
      <div className="max-w-6xl mx-auto">

        {/* Header */}
        <div className="mb-12 reveal">
          <p
            className="text-[10px] font-bold tracking-[0.2em] uppercase mb-3"
            style={{ color: "var(--color-primary)" }}
          >
            Reviews
          </p>
          <h2
            className="font-display font-bold text-3xl md:text-4xl"
            style={{ color: "var(--color-secondary)" }}
          >
            What They&apos;re Saying
          </h2>
        </div>

        {/* Cards */}
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {config.testimonials.map((t, i) => (
            <div
              key={t.name}
              className="reveal flex flex-col rounded-2xl p-6 sm:p-7"
              style={{
                backgroundColor: "var(--color-accent)",
                border: "1px solid var(--color-primary)15",
                transitionDelay: `${i * 0.1}s`,
              }}
            >
              {/* Stars */}
              <div className="flex gap-1 mb-4">
                {Array.from({ length: 5 }).map((_, j) => (
                  <svg key={j} className="w-3.5 h-3.5" viewBox="0 0 20 20" fill={j < t.stars ? "var(--color-primary)" : "#e5e7eb"}>
                    <path d="M9.049 2.927c.3-.921 1.603-.921 1.902 0l1.07 3.292a1 1 0 00.95.69h3.462c.969 0 1.371 1.24.588 1.81l-2.8 2.034a1 1 0 00-.364 1.118l1.07 3.292c.3.921-.755 1.688-1.54 1.118l-2.8-2.034a1 1 0 00-1.175 0l-2.8 2.034c-.784.57-1.838-.197-1.539-1.118l1.07-3.292a1 1 0 00-.364-1.118L2.98 8.72c-.783-.57-.38-1.81.588-1.81h3.461a1 1 0 00.951-.69l1.07-3.292z" />
                  </svg>
                ))}
              </div>

              {/* Quote */}
              <p
                className="text-sm leading-[1.8] flex-1 mb-5"
                style={{ color: `${config.colors.text}80` }}
              >
                &ldquo;{t.text}&rdquo;
              </p>

              {/* Author */}
              <div className="flex items-center gap-3">
                <div
                  className="w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold shrink-0"
                  style={{ backgroundColor: "var(--color-primary)" }}
                >
                  {t.name[0]}
                </div>
                <span
                  className="text-sm font-semibold"
                  style={{ color: "var(--color-secondary)" }}
                >
                  {t.name}
                </span>
              </div>
            </div>
          ))}
        </div>
      </div>
    </section>
  );
}
